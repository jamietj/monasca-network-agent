    ...
    def featExtract(self, q):
        ...
        while Ture:     
            ...
            for ts, pkt in pcap.pcap(name='eth1', immediate=True, timeout_ms=60000):
                ...
                flowStr = ip_to_str(ip.src) + ip_to_str(ip.dst) + str(trans.sport) + str(trans.dport)
                ...
                # src IP
                if ip_to_str(ip.src) not in currentStats['srcIPdist']:
                    currentStats['srcIPdist'][ip_to_str(ip.src)] = 0
                currentStats['srcIPdist'][ip_to_str(ip.src)] += 1

                # dst IP
                if ip_to_str(ip.dst) not in currentStats['dstIPdist']:
                    currentStats['dstIPdist'][ip_to_str(ip.dst)] = 0
                currentStats['dstIPdist'][ip_to_str(ip.dst)] += 1
